...
public class TextUtil {
    ...
    public static String millisToString(long millis, boolean shorten) {
        ...
        if (days > 0) {
            sb.append(days);
            sb.append(shorten ? "d " : " Days ");
        }
        if (hours > 0) {
            sb.append(hours);
            sb.append(shorten ? "h " : " Hours ");
        }
        if (minutes > 0) {
            sb.append(minutes);
            sb.append(shorten ? "m " : " Minutes ");
        }
        sb.append(seconds);
        sb.append(shorten ? "s" : " Seconds");

        return (sb.toString());
    }

    public static String buildBanMessage(HashMap<String, Object> bdata) {
        String time = (bdata.get("type").equals("permanent")) ? "Forever" : TextUtil.millisToString((long) bdata.get("time_end") - System.currentTimeMillis(), false);
    ...
    public static String buildFromArray(int start, String[] array) {
