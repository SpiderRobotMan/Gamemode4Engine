...
public class DatabaseManager {
    ...
    public boolean unbanPlayer(String name) {
        try {
            ...
            PreparedStatement ps = connection.prepareStatement("UPDATE server_bans SET void = 1 WHERE uuid = ?;");
            ps.setString(1, uuid);
    ...
    public HashMap<String, Object> fetchPlayerBans(UUID uuid) {
        try {
            this.connection = openConnection();
            PreparedStatement ps = this.connection.prepareStatement("SELECT * FROM server_bans WHERE uuid = ? AND void = 0;");
            ps.setString(1, uuid.toString());
            ResultSet res = ps.executeQuery();
